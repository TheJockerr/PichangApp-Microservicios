...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `christophacham-agent-skills-library-spring-boot-3`                                |
| Version    | 1.0.1                                     |
| Author     | christophacham                                 |
...
**GitHub:** [christophacham/agent-skills-library](https://github.com/christophacham/agent-skills-library) — ⭐  | Forks: 
...
This Skill codifies Spring Boot 3+ best practices for building services and APIs, focusing on constructor-only dependency injection, typed @ConfigurationProperties with validation, and placing @Transactional boundaries on service-layer beans. Use it when creating or refactoring Spring Boot applications to improve testability, immutability, and separation of concerns. Key features include validated record-based configuration properties scanned via @ConfigurationPropertiesScan, service classes that receive dependencies through constructors, DTO records for REST endpoints, and transactional methods on application services rather than controllers. Typical use cases: wiring beans cleanly, centralizing and validating configuration, implementing REST controllers and service layers, and ensuring consistent transaction semantics. Core advantages are safer configuration, easier unit testing, clearer architecture, and reduced runtime surprises.
...
```bash
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3
```
...
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3 --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3 --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3 --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3 --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install christophacham-agent-skills-library-spring-boot-3 --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate christophacham-agent-skills-library-spring-boot-3 --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment christophacham-agent-skills-library-spring-boot-3 -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment christophacham-agent-skills-library-spring-boot-3
```
...
```bash
npx -y @lobehub/market-cli skills comments christophacham-agent-skills-library-spring-boot-3
```
