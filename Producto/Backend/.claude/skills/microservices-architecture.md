...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `travisjneuman-.claude-microservices-architecture`                                |
| Version    | 1.0.2                                     |
...
**GitHub:** [travisjneuman/.claude](https://github.com/travisjneuman/.claude) — ⭐ 12 | Forks: 1
...
This Skill is a practical guide to designing, implementing, and operating microservices-based systems. It covers core concepts—independently deployable services, single responsibility, data ownership, API-driven communication, and bounded contexts—plus design patterns such as API Gateway, domain-driven design, and event-driven integration. Use it when breaking up monoliths, scaling specific capabilities, enabling parallel teams, or adopting heterogeneous technology stacks. It details deployment and operational best practices: CI/CD pipelines, containerization, observability (logging, metrics, distributed tracing), resilience patterns (circuit breakers, bulkheads, retries), and testing strategies. Key advantages include independent releases, targeted scalability, fault isolation, and faster team autonomy. It also explains when microservices are inappropriate (small apps, low DevOps maturity, strict latency constraints) and offers pragmatic guidance for incremental migration.
...
```bash
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture
```
...
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install travisjneuman-.claude-microservices-architecture --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate travisjneuman-.claude-microservices-architecture --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment travisjneuman-.claude-microservices-architecture -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment travisjneuman-.claude-microservices-architecture
```
...
```bash
npx -y @lobehub/market-cli skills comments travisjneuman-.claude-microservices-architecture
```
