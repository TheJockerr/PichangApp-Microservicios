...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `claude-dev-suite-claude-dev-suite-spring-cloud-gateway`                                |
| Version    | 1.0.2                                     |
...
**GitHub:** [claude-dev-suite/claude-dev-suite](https://github.com/claude-dev-suite/claude-dev-suite) — ⭐ 5 | Forks: 
...
Spring Cloud Gateway provides an API gateway for Spring Boot microservices, offering dynamic routing, request/response filtering, load balancing, and resiliency features. It supports YAML-based route configuration with predicates (Path, Method, Header, Query, Host, Time), filters (StripPrefix, AddRequestHeader/ResponseHeader, DedupeResponseHeader), rate limiting, and Circuit Breaker with fallback URIs. Service discovery integrates via lb:// URIs and Eureka discovery locator for dynamic backends. Use it to centralize authentication/authorization, CORS, observability headers, request transformation, rate limiting, and fault isolation for microservice architectures. Core advantages include native Spring integration, reactive non-blocking routing, extensible custom filters, and declarative configuration. Do not use it as a simple static reverse proxy—prefer nginx for basic proxying. Recommended when building resilient, policy-driven API gateways in Spring ecosystems.
...
```bash
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway
```
...
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install claude-dev-suite-claude-dev-suite-spring-cloud-gateway --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate claude-dev-suite-claude-dev-suite-spring-cloud-gateway --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment claude-dev-suite-claude-dev-suite-spring-cloud-gateway -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment claude-dev-suite-claude-dev-suite-spring-cloud-gateway
```
...
```bash
npx -y @lobehub/market-cli skills comments claude-dev-suite-claude-dev-suite-spring-cloud-gateway
```
