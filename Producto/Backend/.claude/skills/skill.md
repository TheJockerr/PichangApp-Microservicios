...
Java Spring Boot microservices expert for code gen, architecture, security, DB integration (RDBMS/NoSQL). Use for "Spring Boot microservice", "REST API Spring", "JWT auth", "Spring Data JPA", "microservices Eureka".

| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `piyushmakhija28-claude-global-library-java-spring-boot-microservices`                                |
| Version    | 1.0.0                                     |
...
**GitHub:** [piyushmakhija28/claude-global-library](https://github.com/piyushmakhija28/claude-global-library) — ⭐  | Forks: 
...
This skill provides expert guidance, code generation, debugging help, and best-practice recommendations for building scalable, secure, production-ready Java Spring Boot microservices (Java 17+). It generates layered applications (controller, service, repository), DTOs, validation, global exception handling, and enforces keeping business logic out of controllers. It covers microservice patterns—service discovery (Eureka or equivalent), API Gateway, inter-service communication (REST/Feign), centralized configuration, distributed tracing and logging, and stateless design. Data guidance follows rdbms-core and nosql-core constraints, with Spring Data JPA, Spring Data MongoDB, Elasticsearch limited to search/analytics, transaction management, pagination/sorting/filtering, and migration awareness (Flyway/Liquibase). Security features include JWT authentication, role-based authorization, and Spring Security configuration (no security logic in controllers). Use it to scaffold APIs, secure services, integrate RDBMS/NoSQL, and prepare deployments with observability and production-ready conventions.
...
```bash
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices
```
...
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install piyushmakhija28-claude-global-library-java-spring-boot-microservices --version 1.0.0
```
...
```bash
npx -y @lobehub/market-cli skills rate piyushmakhija28-claude-global-library-java-spring-boot-microservices --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment piyushmakhija28-claude-global-library-java-spring-boot-microservices -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment piyushmakhija28-claude-global-library-java-spring-boot-microservices
```
...
```bash
npx -y @lobehub/market-cli skills comments piyushmakhija28-claude-global-library-java-spring-boot-microservices
```
