...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `g1joshi-agent-skills-api-gateway`                                |
| Version    | 1.0.1                                     |
| Author     | G1Joshi                                 |
...
**GitHub:** [G1Joshi/Agent-Skills](https://github.com/G1Joshi/Agent-Skills) — ⭐ 5 | Forks: 
...
An API Gateway Skill implements the centralized ingress pattern for microservices, routing external and internal requests to the correct backend services while handling cross-cutting concerns. Key features include path- and host-based routing, SSL termination, authentication and authorization, rate limiting and quota enforcement, load balancing, caching, request/response transformation, protocol translation (HTTP/gRPC), aggregation/composition of multiple backend responses, circuit breaking and retries, and observability hooks (logging, metrics, distributed tracing). Use it as the single entry point for microservice architectures, mobile or web backends, BFFs, multi-version canary routing, and API monetization. Core advantages are decoupling clients from service topology, centralizing security and traffic policies, simplifying client implementations, and giving operators a single place to enforce observability and resiliency policies. Integrates with service discovery and can be deployed as managed gateways or cloud-native proxies.
...
```bash
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway
```
...
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install g1joshi-agent-skills-api-gateway --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate g1joshi-agent-skills-api-gateway --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment g1joshi-agent-skills-api-gateway -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment g1joshi-agent-skills-api-gateway
```
...
```bash
npx -y @lobehub/market-cli skills comments g1joshi-agent-skills-api-gateway
```
