...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `jander99-skills-spring-boot-core`                                |
| Version    | 1.0.2                                     |
| Author     | jander99                                 |
...
**GitHub:** [jander99/skills](https://github.com/jander99/skills) — ⭐ 1 | Forks: 
...
spring-boot-core helps developers build and structure Spring Boot microservices using proven patterns: constructor-based dependency injection for immutability and testability; typed @ConfigurationProperties with @Validated for safe, externalized configuration; and profile-based configuration to separate environment-specific settings. It also covers Actuator-driven observability—enabling /actuator/health, custom HealthIndicator/HealthContributor implementations, readiness/liveness distinctions, and endpoint security. For lifecycle and resource management, the Skill demonstrates bean scopes, @PostConstruct/@PreDestroy, SmartLifecycle, BeanPostProcessor hooks, and graceful shutdown practices. Use this Skill when creating services, configuring beans and properties, implementing DI patterns, adding health checks, or organizing microservice modules. Core advantages include stronger configuration validation, clearer DI patterns, improved observability, predictable bean lifecycle behavior, and easier testing and deployment across environments.
...
```bash
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core
```
...
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install jander99-skills-spring-boot-core --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate jander99-skills-spring-boot-core --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment jander99-skills-spring-boot-core -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment jander99-skills-spring-boot-core
```
...
```bash
npx -y @lobehub/market-cli skills comments jander99-skills-spring-boot-core
```
