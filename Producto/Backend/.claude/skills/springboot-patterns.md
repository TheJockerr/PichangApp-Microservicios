...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `neversight-skills_feed-springboot-patterns`                                |
| Version    | 1.0.2                                     |
| Author     | NeverSight                                 |
...
springboot-patterns codifies Spring Boot and Java best practices for building maintainable, testable REST APIs, services, and repositories using Domain-Driven Design (DDD). Use it when designing application and domain layers, defining aggregate roots and repositories, or enforcing transactional boundaries with @Transactional. Key features include layered architecture guidance (controllers, application services, domain services, repositories), DTO mapping and validation (MapStruct, Jakarta Bean Validation), consistent error handling and API versioning, pagination and caching patterns, security and configuration profile recommendations, and testing strategies (unit, integration, Testcontainers). It also prescribes logging, metrics, resilience (circuit breakers, retries), and code style conventions for naming, immutability, and dependency management. Core advantages are improved consistency, fewer defects, easier team onboarding, and scalable, maintainable Java microservices.
...
```bash
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns
```
...
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install neversight-skills_feed-springboot-patterns --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate neversight-skills_feed-springboot-patterns --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment neversight-skills_feed-springboot-patterns -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment neversight-skills_feed-springboot-patterns
```
...
```bash
npx -y @lobehub/market-cli skills comments neversight-skills_feed-springboot-patterns
```
